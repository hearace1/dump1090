var planeObject = {
	...
	funcClearLine	: function() {
			if (this.line) {
				LeafletMap.removeLayer(this.line);
				this.line = null;
			}
		},

	// Builds the rotated plane icon (a Leaflet divIcon holding an inline SVG).
	funcGetIcon	: function() {
			...
			// Plane marker path
			var planeData = "M 1.9565564,41.694305 C 1.7174505,40.497708 1.6419973,38.448747 " +
			...
			var strokeWeight = (this.is_selected ? 2 : 1);
			var rotation = (this.track ? this.track : 0);

			var svg = '<svg width="40" height="40" viewBox="-10 -10 84 84" style="overflow:visible;">' +
				'<path transform="rotate(' + rotation + ' 32 32)" d="' + planeData + '" ' +
				'fill="' + this.markerColor + '" fill-opacity="0.9" stroke="#000000" ' +
				'stroke-width="' + strokeWeight + '" vector-effect="non-scaling-stroke" /></svg>';

			return L.divIcon({
				className : 'plane_icon',
				html      : svg,
				iconSize  : [40, 40],
				iconAnchor: [20, 20]   // anchor to middle of plane
			});
		},
	...
	funcUpdateData	: function(data){
			...
			if (this.seen > 58) {
				this.reapable = true;
				if (this.marker) {
					LeafletMap.removeLayer(this.marker);
					this.marker = null;
				}
				if (this.line) {
					LeafletMap.removeLayer(this.line);
	...
	funcUpdateMarker: function() {
			...
			var title = (this.flight && this.flight.length > 0)
